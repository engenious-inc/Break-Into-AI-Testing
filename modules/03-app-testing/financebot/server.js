#!/usr/bin/env node
// FinanceBot GenAI demo — HTTP surface.
//
//   GET  /        -> workshop chat UI (public/index.html)
//   GET  /health  -> {"status":"ok", ...}   run this before an eval
//   POST /chat    -> {answer, route, citations, debug}
//
// Usage: node modules/03-app-testing/financebot/server.js   (or ./run.sh financebot-serve)
// Port:  FINANCEBOT_PORT, default 8001 (PayFlow uses 8000).
...
const PORT = Number(process.env.FINANCEBOT_PORT || 8001);
...
server.listen(PORT, () => {
  console.log(`FinanceBot GenAI demo listening on http://localhost:${PORT}`);
