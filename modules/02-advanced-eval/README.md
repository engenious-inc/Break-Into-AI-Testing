...
Groq; cost budgets and embedding-based checks are opt-in (need a paid key), and are
marked per lesson.
...
- `structured-outputs/` — JSON Schema via `is-json` / `contains-json`, plus `options.transform`
  to unwrap markdown fences so the same contract still holds
...
- `observability/` — **Day 8**, not Day 4. Real Arato.ai REST logging + hand-rolled
  OTel-shaped tracing via a custom zero-dependency provider; shows what production
  telemetry adds beyond eval-time quality checks.

> Maps to `how-to-test-ai` day-03-promptfoo-advanced, and to the August cohort's
> **Day 4 (Model-Graded Assertions & Metrics)** — with one exception. `observability/`
> lives here because it is a Module 2-shaped lesson (single provider, ordinary
> pass=good assertions), but it is taught on **Day 8** alongside Arato.ai and Agenta.ai.
> Do not assign it with the Day 4 exercises.
>
> Day 4 exercises are written up in
